...
UnlockServer::UnlockServer(std::string ip, int port,
                           std::string encryptionPwd, std::string unlockToken) : BaseUnlockServer(encryptionPwd, unlockToken) {
    m_IsRunning = false;
...
bool UnlockServer::Start() {
    ...
    m_IsRunning = true;
    m_AcceptThread = std::thread(&UnlockServer::AcceptThread, this);

    //Logger::println("Server started.\n");
    return true;
}

void UnlockServer::Stop() {
    if (!m_IsRunning)
        return;
...
void UnlockServer::AcceptThread() {
